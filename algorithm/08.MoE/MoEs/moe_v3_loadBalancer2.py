...
class MoE(nn.Module):
    def __init__(self, input_dim, output_dim, num_experts, topk=2):
        ...
        # 모든 전문가를 하나의 큰 선형 계층으로 통합 -> 병렬 계산을 위해
    ...
    def forward(self, x):
        ...
        # 7. 보조 손실 계산 (로드 밸런싱)
        # 각 전문가에 대한 선택 빈도를 계산
        # top_k_indices는 [batch_size, topk], 이를 모두 flatten하여 [batch_size * topk]
        all_selected = top_k_indices.view(-1)  # [batch_size * topk]
        expert_load = torch.bincount(all_selected, minlength=self.num_experts).float()  # [num_experts]
        ...
        cv = load_std / (load_mean + 1e-8)  # [scalar]
        ...
        return output, aux_loss  # [batch_size, output_dim], scalar
...
if __name__ == "__main__":
    ...
    model = MoE(input_dim, output_dim, num_experts, topk=topk, lambda_aux=lambda_aux)
    x = torch.randn(batch_size, input_dim)
    output, aux_loss = model(x)
    print(output.shape)      # 예상 출력: [32, 256]
    print(aux_loss.item())   # 보조 손실 출력 (스칼라 값)
