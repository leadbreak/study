...
class MoE(nn.Module):
    ...
    def forward(self, x):
        ...
        # 게이트 네트워크를 통해 각 전문가의 선택 확률 계산
        gate_outputs = self.gate(x)  # [batch_size, num_experts]
        
        # 로드 밸런싱 로스 계산
        importance = gate_outputs.mean(dim=0)  # [num_experts]
        load_balance_loss = (importance * importance).sum() * self.num_experts  # 스케일링
...
if __name__ == "__main__":
    ...
    print(load_balance_loss.item())  # 로드 밸런싱 로스 값 출력
