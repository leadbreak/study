...
###############################################
# Model 정의 (ModernTransformer → 최종 LLaMA)
###############################################
...
class LLaMA:
    """
    LLaMA 클래스는 학습과 추론에 모두 사용할 수 있도록 ModernTransformer와 토크나이저를 포함합니다.
    """
    ...
    @torch.inference_mode()
    def _generate_beam(self, tokens: torch.Tensor, max_new_tokens: int, beam_width: int, temperature: float) -> torch.Tensor:
        self.model.eval()
        ...
        for _ in range(max_new_tokens):
            ...
            for seq, score in beams:
                logits = self.model.forward_inference(seq)
