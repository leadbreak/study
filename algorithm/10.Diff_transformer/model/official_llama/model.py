...
# --- Single GPU replacements for fairscale components ---
def get_model_parallel_world_size():
    return 1

# For single GPU usage, we simply use standard nn.Linear and nn.Embedding.
...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.n_local_heads = args.n_heads // model_parallel_size  # equals args.n_heads
        self.n_local_kv_heads = self.n_kv_heads // model_parallel_size  # equals self.n_kv_heads
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int):
        # Inference (with caching) remains unchanged.
    ...
    def forward_train(self, tokens: torch.Tensor, start_pos: int):
        # Training: 캐시를 사용하지 않도록 매 layer마다 임시 캐시를 0으로 초기화합니다.
        _bsz, seqlen = tokens.shape
        h = self.tok_embeddings(tokens)
        self.freqs_cis = self.freqs_cis.to(h.device)
        freqs_cis = self.freqs_cis[start_pos: start_pos + seqlen]
        ...
        for layer in self.layers:
            # 임시로 캐시를 0으로 초기화하여, backward 시 이전 그래프를 참조하지 않도록 함.
